...
namespace Sd.Update
{
    public partial class MainForm : Form
    {
        ...
        private string _callBackPath = ""; //自动升级完成后，要启动的exe的完整路径。        
        private bool startAppAfterClose = false; //关闭升级窗体前，是否启动应用程序。
        private string _serverIP ;
        private int _serverPort;
        private string _callBackExeName; //自动升级完成后，要启动的exe的名称。
        private string _title;

        public MainForm(string serverIP, int serverPort, string callBackExeName, string title)
        {
            InitializeComponent();
            _serverIP = serverIP;
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                _callBackPath = dir.FullName + "\\" + _callBackExeName; //自动升级完成后，要启动的exe的完整路径。（1）被分发的程序的可执行文件exe必须位于部署目录的根目录。（2）OAUS的客户端（即整个AutoUpdater文件夹)也必须位于这个根目录。

                this.Text = _title;

                this.updater = new Updater(_serverIP, _serverPort);
        ...
        private void MainForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                string processName = _callBackExeName.Substring(0, _callBackExeName.Length - 4);
                ...
                if (File.Exists(this._callBackPath))
                {
                    System.Diagnostics.Process myProcess = System.Diagnostics.Process.Start(this._callBackPath);
                }
            }
            catch (Exception ee)
            {
